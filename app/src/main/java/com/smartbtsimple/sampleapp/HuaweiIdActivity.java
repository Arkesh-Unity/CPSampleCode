...
import com.smartbtsimple.logger.Log;
import com.smartbtsimple.logger.LoggerActivity;
import com.smartbtsimple.sampleapp.common.ICallBack;
...
public class HuaweiIdActivity extends LoggerActivity implements OnClickListener {
    ...
    public static final String Hash_Key = "qIkiT/YNqbx";
    private static final int REQ_USER_CONSENT = 201;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // For SMS Permission, This is sample so you have to allow first time
        requestPermission();
        // For getting hashkey...you can see logcat and delete AppSignatureHelper file
        AppSignatureHelper hashkey = new AppSignatureHelper(this);
        hashkey.getAppSignatures();
...
// For Runtime Permission
    private void requestPermission() {
        ActivityCompat.requestPermissions(this, new String[]{Manifest.permission.READ_SMS, Manifest.permission.SEND_SMS, Manifest.permission.RECEIVE_SMS}, PERMISSION_REQUEST_CODE);
    }
    ...
    /**
     * Codelab Code
     * start ReadSmsManager
     * sendSMS is for sending message here, you can use your network call
     */
    private void startServicAndGetOtp() {
        Task<Void> task = ReadSmsManager.start(HuaweiIdActivity.this);
        task.addOnSuccessListener(new OnSuccessListener<Void>() {
            @Override
            public void onSuccess(Void aVoid) {
                Log.d("task ",  "Service Started ");
            }
        });
        task.addOnFailureListener(new OnFailureListener() {
            @Override
            public void onFailure(Exception e) {
                Log.d("task result ", e.getMessage());
            }
        });
        task.addOnCompleteListener(new OnCompleteListener<Void>() {
            ...
            public void onComplete(Task<Void> task) {
                if (task.isSuccessful()) {
                    ...
                    Toast.makeText(HuaweiIdActivity.this, "Successfully Done.  " + task.getResult(), Toast.LENGTH_SHORT).show();

                    // Don't Use Thread sleep in practise...its for sample only
                    try {
                        Thread.sleep(2000);
    ...
    public void sendSMS() {
        String number = "9034824024";  // The number on which you want to send SMS
        PendingIntent sentIntent = null, deliveryIntent = null;
        // Use SmsManager.
        SmsManager smsManager = SmsManager.getDefault();
        smsManager.sendTextMessage
                (number, number, "<#> Your OTP is: 156367 "+Hash_Key,
    ...
    // Register Broadcast Receiver also follow onStart() and onStop()
